...
namespace DoAn3DVH.Models
{
    public class SanPhamModel
    {
        ...
        public List<SanPham> SPquangcao()
        {
            DataTable dt = db.GetData("SELECT * FROM DienThoai WHERE trangThai LIKE 'quangcao'");
        ...
        public List<SanPham> timKiemTen(string id)
        {
            DataTable dt = db.GetData(string.Format("SELECT * FROM DienThoai WHERE tenSp LIKE '%{0}%'", id));
            List<SanPham> li = new List<SanPham>();
            foreach (DataRow dr in dt.Rows)
            {
                SanPham nv = new SanPham();
