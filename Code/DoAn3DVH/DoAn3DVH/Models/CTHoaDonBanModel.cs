...
namespace DoAn3DVH.Models
{
    public class CTHoaDonBanModel
    {
        ...
        public List<ChiTietHoaDonBan> GetListChiTiet(string id)
        {
            DataTable dt = db.GetData("SELECT CT.*, DT.tenSp, DT.hinhAnh FROM chiTietHoaDonBan CT INNER JOIN DienThoai DT ON CT.maSp = DT.maSp WHERE maHoaDonBan = '" + id + "'");
            List<ChiTietHoaDonBan> li = new List<ChiTietHoaDonBan>();
            foreach (DataRow dr in dt.Rows)
            {
                ChiTietHoaDonBan l = new ChiTietHoaDonBan();
                ...
                l.tenSp = dr[4].ToString();
                l.hinhAnh = dr[5].ToString();
                li.Add(l);
            }
            return li;
        }
