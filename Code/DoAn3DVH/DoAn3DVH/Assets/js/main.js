...
(function($) {
    ...
    ******************************************** */
    ...
    $(".qtybutton").on("click", function() {
        ...
        var idValue = $button.parent().find("input").attr("data-id");

        var tongTien = $button.parent().parent().parent().find("#tongTien");

        var dori = 0;
        if ($button.text() == "+")
            dori = 1;

        $.get("/Home/thayDoiSoLuong", { id: idValue , dori: dori}, function (data) {
            if (data.success == true) {
                $button.parent().find("input").val(data.soluong);
                $(tongTien).html(formatNumber(data.tongtien, ".", ",") + " VNĐ");
            }
        });
    });
